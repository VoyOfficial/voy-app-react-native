...
**PR integration for voy-app-react-native:** For how to use spec-driven-eval as a PR quality gate in this project (GitHub-based, no Azure DevOps), see [pr-integration.md](references/pr-integration.md).
...
A complete applied evaluation (a billing service example's P0 "Start Free Trial Without a Card") showing the binary checklist per AC with `file:line` evidence, plus calibration anchors, is in [reference.md](references/reference.md). Mirror its structure and rigor — the methodology is stack-agnostic, apply it to this project's Clean Architecture layers (domain/data/presentation) instead.
